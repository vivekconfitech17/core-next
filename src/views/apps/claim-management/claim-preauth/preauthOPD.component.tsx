...
export default function ClaimsPreAuthOPDComponent(props: any) {
  ...
  const handleInputChangeProvider = (e: any, index: number) => {
  ...
  const handleBenefitChangeInService = (e: any, index: any) => {
    const isValAlreadyPresent = serviceDetailsList.some((item: any) => item.benifitId === e.value)
  ...
  const handleEstimateChangeProvider = (i: any, idx: any, e: any) => {
      ...
      <Paper elevation={0}>
        <Box p={3} my={2}>
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <Grid container spacing={3} style={{ marginBottom: '20px' }}>
              ...
              <Grid item xs={12} sm={3} style={{ display: 'flex', alignItems: 'flex-end', marginBottom: '8px' }}>
                ...
                <Autocomplete
                  ...
                  style={{ width: '100%' }}
                  getOptionLabel={(option: any) => option.diagnosisName}
                  renderOption={(option: any) => <React.Fragment>{option.diagnosisName}</React.Fragment>}
                  value={selectedId}
                  className={classes.benifitAutoComplete}
            ...
            {serviceDetailsList.map((x: any, i: number) => {
              return (
                      ...
                      <Grid item xs={12} sm={6} md={3}>
                        <FormControl className={classes.formControl} fullWidth>
                          <Autocomplete
                            defaultValue={x?.benefitId}
                            value={x?.benefitId}
                            onChange={(e, val) => handleBenefitChangeInService(val, i)}
                            id='checkboxes-tags-demo'
                            filterOptions={autocompleteFilterChange}
                            options={selectedBenefit}
                            getOptionLabel={(option: any) =>
