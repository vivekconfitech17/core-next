...
export default function ClaimsPreAuthIPDComponent(props: any) {
  ...
  const [serviceLists, setServiceLists] = React.useState<any>({})
  ...
  const handleBenefitChangeInService = (e: any, index: any) => {
    ...
    if (!isValAlreadyPresent) {
      ...
      if (index >= 0 && index < list.length) {
        list[index] = { ...list[index], benefitId: e.benefitStructureId ? e.benefitStructureId : '' } // Ensure the object is updated immutably
        setServiceDetailsList(list)
      } else {
  ...
  const getServices = async (interventionCode: any, benefitStructureId: any, index: any) => {
    const result: any = await benefitService
      .getServicesfromInterventions(interventionCode, benefitStructureId)
      .toPromise()
    ...
    setServiceLists((prev: any) => ({ ...prev, [index]: temp }))
  }

  useEffect(() => {
    serviceDetailsList.forEach((x: any, index: any) => {
      if (x.benefitId) {
        getInterventions(x.benefitId, index)
      }

      if (x.interventionCode && x.benefitId) {
        getServices(x.interventionCode, x.benefitId, index)
      }
    })
  }, [serviceDetailsList])
      ...
      <Paper elevation={0}>
        <Box p={3} my={2}>
          ...
          <form onSubmit={formik.handleSubmit}>
                        ...
                        {memberName?.res?.content && memberName?.res?.content?.length > 0 ? (
                          <TableContainer>
                            <Table>
                              ...
                              <TableBody>
                                {memberName.res.content.map((item: any, index: number) => (
            ...
            <Grid item xs={12} sm={12} md={12} style={{ marginTop: '20px', marginBottom: '15px' }}>
              <Divider />
            </Grid>
            ...
            <Grid container spacing={3} style={{ marginBottom: '20px' }}>
              ...
              <Grid item xs={12} sm={3} style={{ display: 'flex', alignItems: 'flex-end', marginBottom: '8px' }}>
                ...
                {/* <Autocomplete
                  ...
                  style={{ width: '100%' }}
                  getOptionLabel={option => option.diagnosisName}
                  renderOption={option => <React.Fragment>{option.diagnosisName}</React.Fragment>}
                  value={selectedId}
                  className={classes.benifitAutoComplete}
            ...
            {serviceDetailsList?.map((x, i) => {
              return (
                <ServiceDetails
                  ...
                  serviceList={serviceLists[i] || []}
                />
              )
            })}

            <Grid item xs={12} sm={12} md={12} style={{ marginBottom: '15px', marginTop: '10px' }}>
              <Divider />
            </Grid>
