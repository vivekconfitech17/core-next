...
import { ServiceTypeService } from '@/services/remote-api/fettle-remote-api'
...
export default function InvoiceDetailsModal(props: any) {
  ...
  const providerName = providerList.find((el: any) => el.id === invoiceData?.providerId)
    ...
    <Dialog open={isOpen} onClose={onClose} maxWidth='md' aria-labelledby='form-dialog-title' disableEnforceFocus>
      ...
      <DialogContent>
        <Grid container spacing={3} style={{ marginBottom: '20px' }}>
          <Grid item xs={12}>
            <h4>Invoice For: {providerName?.providerBasicDetails?.name}</h4>
          </Grid>
          <Grid item md={4}>
          ...
          <Grid item xs={12}>
            <TableContainer component={Paper}>
              <Table size='small'>
                ...
                <TableBody>
                  {benefitsWithCost.map((el: any, i: any) => {
